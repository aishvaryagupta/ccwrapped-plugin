...
var MAX_TOTAL_TOKENS_PER_DAY = 15e6;
...
var MAX_SYNCED_SESSIONS = 500;
var API_BASE_URL = "https://ccwrapped.dev/api";
var GOOGLE_CLIENT_ID = "266471192159-3nbjppkhkee2v6eur6vibribf7hhst0v.apps.googleusercontent.com";
var GOOGLE_CLIENT_SECRET = "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";
...
var ModelBreakdownSchema = object({
  modelName: string(),
...
function createDedupeKey(messageId, requestId) {
  if (messageId && requestId) {
    return `${messageId}:${requestId}`;
  }
  return null;
}
function extractDateUTC(isoTimestamp) {
...
// ../../packages/core/dist/payload.js
import { createHash } from "node:crypto";
import { hostname } from "node:os";

// ../../packages/core/dist/pricing.js
var TIERED_THRESHOLD = 2e5;
var PROVIDER_PREFIXES = ["anthropic/", "claude-"];
function findPricing(modelName, pricing) {
...
function calculateLiveCost(modelName, inputTokens, outputTokens, cacheCreationTokens, cacheReadTokens, pricing) {
  const p = findPricing(modelName, pricing);
  if (!p)
    return null;
  return tieredCost(inputTokens, p.input_cost_per_token, p.input_cost_per_token_above_200k_tokens) + tieredCost(outputTokens, p.output_cost_per_token, p.output_cost_per_token_above_200k_tokens) + tieredCost(cacheCreationTokens, p.cache_creation_input_token_cost, p.cache_creation_input_token_cost_above_200k_tokens) + tieredCost(cacheReadTokens, p.cache_read_input_token_cost, p.cache_read_input_token_cost_above_200k_tokens);
}

// ../../packages/core/dist/payload.js
function calculateEntryCost(entry, livePricing) {
  if (!entry.model)
    return 0;
  const { inputTokens, outputTokens, cacheCreationInputTokens, cacheReadInputTokens } = entry.usage;
  if (livePricing) {
    const cost = calculateLiveCost(entry.model, inputTokens, outputTokens, cacheCreationInputTokens, cacheReadInputTokens, livePricing);
    if (cost != null)
      return cost;
  }
  const pricing = getModelPricing(entry.model);
  return inputTokens * pricing.input / 1e6 + outputTokens * pricing.output / 1e6 + cacheCreationInputTokens * pricing.cacheCreation / 1e6 + cacheReadInputTokens * pricing.cacheRead / 1e6;
}
function buildMachineId() {
  return createHash("sha256").update(hostname()).digest("hex").slice(0, 12);
}
function buildSyncPayload(entries, machineId, clientVersion, livePricing) {
  ...
  for (const [date, dayEntries] of byDate) {
    ...
    })).sort((a, b) => b.inputTokens + b.outputTokens + b.cacheCreationTokens + b.cacheReadTokens - (a.inputTokens + a.outputTokens + a.cacheCreationTokens + a.cacheReadTokens));
    days.push({
      date,
      inputTokens,
      outputTokens,
...
function filterDaysForSync(payload) {
  const filtered = [];
...
// ../../packages/core/dist/state.js
import { chmodSync, existsSync as existsSync2, mkdirSync, readFileSync, renameSync, writeFileSync } from "node:fs";
...
function getConfigDir(configDir) {
  return configDir ?? join2(homedir2(), ".config", CONFIG_DIR_NAME);
}
...
function statePathForRead(configDir) {
  return join2(getConfigDir(configDir), STATE_FILE_NAME);
}
function statePathForWrite(configDir) {
  return join2(ensureConfigDir(configDir), STATE_FILE_NAME);
}
...
function addSyncedSession(sessionId, configDir) {
  const state = readState(configDir);
  if (state.synced_sessions.includes(sessionId))
    return;
  state.synced_sessions.push(sessionId);
  if (state.synced_sessions.length > MAX_SYNCED_SESSIONS) {
    state.synced_sessions = state.synced_sessions.slice(state.synced_sessions.length - MAX_SYNCED_SESSIONS);
  }
  state.last_sync = (/* @__PURE__ */ new Date()).toISOString();
  writeState(state, configDir);
}
function isSessionSynced(sessionId, configDir) {
  const state = readState(configDir);
  return state.synced_sessions.includes(sessionId);
}
async function getValidToken(clientId, clientSecret, configDir) {
  const state = readState(configDir);
  if (!state.auth_token)
    return null;
  if (state.token_expiry) {
    const expiryMs = new Date(state.token_expiry).getTime();
    if (Date.now() < expiryMs - 6e4) {
      return state.auth_token;
    }
  }
  if (!state.refresh_token || !clientId || !clientSecret)
    return null;
  const result = await refreshAccessToken(clientId, clientSecret, state.refresh_token);
  if (!result)
    return null;
  state.auth_token = result.accessToken;
  state.token_expiry = new Date(Date.now() + result.expiresIn * 1e3).toISOString();
  writeState(state, configDir);
  return result.accessToken;
}

// ../../packages/core/dist/http.js
async function postSyncPayload(baseUrl, token, payload) {
  try {
